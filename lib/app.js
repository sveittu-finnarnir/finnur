...
const authy = require('./flows/authorisation').authy;
...
function runStateMachine(user, message) {
  ...
  .spread((nextState, reply) => {
    db[user.id] = nextState;
    return reply;
  }).catch((err) => {
    console.error(err);
  });
...
function loggedOutState (input) {
  return Bluebird.resolve([loggedOutState, "you're not logged in"]);
}
