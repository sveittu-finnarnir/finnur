...
function runStateMachine(user, message) {
  let handler = stateMap[user.id] || loggedOutState;
...
function getUser(fbid) {
  ...
  .then(user => {
    if (!user) {
      ...
      .then(user => {
        return knex('Users').insert(user).returning('*').first();
      });
    } else if (!_.has(user, 'first_name')) {
      return facebook.getUserDetails(fbid)
      .then(user => {
        return knex('Users').where('id', fbid).update(user).returning('*').first();
      });
    }
    return user;
  });
...
function loggedOutState (input, user) {
  return flows.authorisation.start(loggedInState).then((nextState) => {
    return [
      nextState,
...
function loggedInState (input, user) {
